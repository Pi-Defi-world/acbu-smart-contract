...
fn test_calculate_fee_1_percent() {
    let amount = 10_000_000i128;
    let fee_rate = 100i128; // 1%
    let expected = 100_000i128; // 1% of 10M
...
fn test_calculate_fee_3_percent() {
    let amount = 10_000_000i128;
    let fee_rate = 300i128; // 3%
    let expected = 300_000i128; // 3% of 10M
    assert_eq!(calculate_fee(amount, fee_rate), expected);
}
...
fn test_calculate_fee_10_percent() {
    ...
    let expected = 1_000_000i128; // 10% of 10M
    assert_eq!(calculate_fee(amount, fee_rate), expected);
}
...
fn test_calculate_fee_100_percent() {
    let amount = 10_000_000i128;
    let fee_rate = BASIS_POINTS; // 100%
    let expected = 10_000_000i128; // 100% of 10M
    assert_eq!(calculate_fee(amount, fee_rate), expected);
}

#[test]
fn test_calculate_fee_large_amount() {
    let amount = 1_000_000_000_000i128; // 1M USDC (7 decimals)
    let fee_rate = 300i128; // 3%
    let expected = 30_000_000_000i128; // 3% of 1M
    assert_eq!(calculate_fee(amount, fee_rate), expected);
}
...
fn test_calculate_fee_small_amount() {
    let amount = 1i128;
    let fee_rate = 300i128; // 3%
    // 1 * 300 / 10000 = 0 (integer division)
    assert_eq!(calculate_fee(amount, fee_rate), 0);
}
...
fn test_calculate_deviation_value2_greater() {
    let value1 = 100i128;
    let value2 = 150i128;
    // (150 - 100) * 10000 / 150 = 3333 bps ≈ 33.33%
...
fn test_calculate_deviation_large_deviation() {
    let value1 = 200i128;
    let value2 = 100i128;
    // (200 - 100) * 10000 / 100 = 10000 bps = 100%
...
fn test_calculate_deviation_zero_divisor() {
    let value1 = 100i128;
    let value2 = 0i128;
    // Division by zero should return i128::MAX
...
fn test_calculate_deviation_both_zero() {
    let value1 = 0i128;
    let value2 = 0i128;
    // Both zero should return i128::MAX (division by zero)
    assert_eq!(calculate_deviation(value1, value2), i128::MAX);
}
...
fn test_calculate_deviation_negative_value1() {
    let value1 = -100i128;
    let value2 = 100i128;
    // (100 - (-100)) * 10000 / 100 = 20000 bps = 200%
    assert_eq!(calculate_deviation(value1, value2), 20_000);
}

#[test]
fn test_calculate_deviation_7decimal_rates() {
    // Test with realistic 7-decimal rate values
    let value1 = 1_050_000i128; // 1.05 USD
    let value2 = 1_000_000i128; // 1.00 USD
    // (1_050_000 - 1_000_000) * 10000 / 1_000_000 = 500 bps = 5%
